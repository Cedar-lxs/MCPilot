...
网络搜索通过博查 AI 的结构化 JSON 接口执行，不再抓取或解析搜索引擎页面。复制 `.env.example` 为 `.env` 后，设置以下变量：

| 变量 | 说明 | 默认值 |
|---|---|---|
| `BOCHA_API_KEY` | 博查 AI API 密钥（必填） | 空 |
| `SEARCH_TIMEOUT_SECONDS` | 单次请求超时秒数 | `10` |
